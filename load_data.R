...
employment <- read_csv(file = "./data/Unemployment.csv")

home_price <- read_csv(file = "./data/County_MedianListingPricePerSqft_AllHomes.csv")
home_price$FIPS <- paste0(home_price$StateCodeFIPS, home_price$MunicipalCodeFIPS)
home_price <- 
  home_price %>% 
...
education_income <- left_join(sum_education_perf, employment, by=c("FIPS"="FIP"))
...
save(one_data_set_z, file = "one_data_set_z.rdata")
